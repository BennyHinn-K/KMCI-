...
const querySchema = z.object({
  ...
  low_stock_only: z.string().optional().transform(val => val === 'true')
})
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Fetch products
    const result = await productsService.getAll(validatedQuery, userInfo?.user.id)

    const duration = Date.now() - startTime
    apiLogger.response('GET', '/api/products', 200, duration, userInfo?.user.id)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const duration = Date.now() - startTime
    apiLogger.response('POST', '/api/products', 201, duration, userInfo.user.id)
    ...
    apiLogger.error('POST', '/api/products', error as Error, userInfo?.user.id, 500)
  ...
  try {
    ...
    switch (action) {
      case 'updateStatus':
        ...
        result = await productsService.bulkUpdateStatus(ids, data.status, userInfo.user.id)
    ...
    const duration = Date.now() - startTime
    apiLogger.response('PUT', '/api/products/bulk', 200, duration, userInfo.user.id)
    ...
    const duration = Date.now() - startTime
    const userInfo = await authenticateUser(request).catch(() => null)
    apiLogger.error('PUT', '/api/products/bulk', error as Error, userInfo?.user.id, 500)
