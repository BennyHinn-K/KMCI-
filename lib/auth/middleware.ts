...
export function generateToken(payload: { userId: string, email: string }): string {
  return verify(payload, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN }) as string
}
...
export async function getUserProfile(userId: string): Promise<Profile | null> {
  try {
    const query = 'SELECT * FROM profiles WHERE user_id = $1 AND is_active = true'
...
export async function updateLoginAttempts(userId: string, increment: boolean = true): Promise<void> {
...
// Register new user
export async function registerUser(userData: z.infer<typeof registerSchema>): Promise<AuthenticatedUser> {
  try {
    // Validate input
    ...
    return await db.transaction(async (client) => {
      ...
      const displayName = validatedData.display_name ||
    ...
    if (error instanceof z.ZodError) {
      throw new AuthenticationError('Invalid registration data', 'VALIDATION_ERROR')
    }

    logger.error('Failed to register user', error as Error, { email: userData.email })
    throw error
  }
}

// Login user
export async function loginUser(credentials: z.infer<typeof loginSchema>, ip?: string): Promise<{
  ...
  try {
    ...
    // Verify password
    const isValidPassword = await verifyPassword(validatedCredentials.password, user.password_hash)
...
export async function authenticateUser(request: NextRequest): Promise<AuthenticatedUser | null> {
  try {
    ...
    if (!profile || !profile.is_active) {
      throw new AuthenticationError('Profile not found or inactive', 'PROFILE_INACTIVE')
    }
...
export function requireRole(profile: Profile, allowedRoles: Profile['role'][]): boolean {
  return allowedRoles.includes(profile.role)
}

// Role hierarchy check (higher roles include lower permissions)
export function hasPermissionLevel(profile: Profile, requiredLevel: Profile['role']): boolean {
...
export async function logAuditEvent(
  ...
  try {
    ...
    logger.error('Failed to log audit event', error as Error, { userId, action, details })
  }
}
...
export async function changePassword(
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new AuthenticationError('Invalid password format', 'INVALID_PASSWORD_FORMAT')
    }
